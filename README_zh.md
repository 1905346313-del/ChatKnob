...
- ✅ **三键操作**：
  - `YES`：将当前字符追加到输入缓冲区末尾
  - `BACK`：短按删除输入缓冲区最后一个字符（退格），长按 2 秒重置 WiFi 并重新配网
  - `SEND`：通过 MQTT 发送完整消息，并清空输入缓冲区
- 🖥️ **OLED 实时显示**：
  - 顶部导航栏：方向指示符 + 相邻字符 + 当前选中字符（居中）
  - 第二行：接收到的聊天消息（`ID: message`）
  - 第三行：正在构建的输入句子
- 🌐 **MQTT 通信**：自动获取 MAC 地址作为唯一设备 ID，支持遗嘱消息（LWT）
- 📡 **WiFi 配网**：首次开机自动开启配网热点（`ChatKnob_AP`），配网期间 OLED 显示热点名称，通过网页一次性配置 WiFi 和 MQTT 参数
- 💾 **MQTT 参数持久化**：配网时填写的 MQTT 服务器、端口、主题自动保存到 Flash，断电不丢失
- 🔁 **自动重连**：WiFi 或 MQTT 断线后自动重连，保证设备长期在线
- 🔄 **重新配网**：长按 `BACK` 键 2 秒，设备自动清除 WiFi 信息并进入配网模式
...
| 组件 | 型号/规格 | 数量 |
| :--- | :--- | :--- |
| **主控板** | ESP32 开发板（支持 WiFi、I2C 和 PCNT） | 1 |
...
2. 选择开发板：`Tools → Board → ESP32 Dev Module`。
3. 选择正确端口，点击 **Upload** 烧录。

> **注意**：WiFi 和 MQTT 配置**无需硬编码**，设备首次开机会自动创建配网热点，通过网页完成所有配置。
...
1. **上电启动**：设备自动创建 `ChatKnob_AP` 热点，OLED 屏幕显示热点名称。
2. **连接热点**：用手机或电脑连接该热点（无密码）。
3. **配网**：浏览器自动弹出配网页，依次配置：
   - WiFi 网络和密码
   - MQTT 服务器地址
   - MQTT 端口
   - MQTT 主题
4. **保存重启**：点击保存，设备自动重启并连接到配置的网络和 MQTT 服务器。
...
- **长按 BACK 键（2 秒）**：设备自动清除保存的 WiFi 信息，重启后进入配网模式，用于更换 WiFi 或 MQTT 配置。
...
│   ├── wifi_init()          // WiFi 配网 + MQTT 参数配置（WiFiManager）
│   ├── generate_mac_id()    // MAC 地址 → 设备 ID
│   ├── mqtt_init()          // MQTT 连接（使用保存的参数）
...
│   └── long_press → menu()  // 长按 BACK：重置 WiFi
├── menu()                   // 重置 WiFi 信息，进入配网模式
├── send_message()           // 统一输出接口
│   ├── mode=1 → MQTT 发布
│   ├── mode=2 → OLED 打印（主字体）
│   ├── mode=3 → OLED 打印（符号字体）
...
├── action()                 // 按键动作执行器
├── mqttCallback()           // 接收消息解析
├── save_mqtt_config()       // MQTT 参数存入 Flash
├── load_mqtt_config()       // MQTT 参数从 Flash 读取
└── 其他辅助函数
```
